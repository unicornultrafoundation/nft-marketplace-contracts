...
const buy2 = async () => {
  ...
  const contractNFT1155Signer3 = new ethers.Contract(ADDRESS_ERC1155U2U, abiERC1155U2U, signer3);
  const txApproveNFT1155 = await contractNFT1155Signer3.setApprovalForAll(ADDRESS_ERC1155_NFT_MARKETPLACE, true);
  await txApproveNFT1155.wait();
  ...
  const txCreateAsk1155Marketplace = await contractERC1155NFTMarketplaceSigner3.createAsk(ADDRESS_ERC1155U2U, tokenId, quantitySell, quoteToken, priceSellPerUnit);
  await txCreateAsk1155Marketplace.wait();

  const contractERC20Signer5 = new ethers.Contract(quoteToken, abiERC20, signer5);
  const txApproveERC20Signer5 = await contractERC20Signer5.approve(ADDRESS_ERC1155_NFT_MARKETPLACE, priceBuy);
  ...
  const txBuy1155Marketplace = await contractERC1155NFTMarketplaceSigner5.buyUsingEth(askId, quantityBuy, {value: priceBuy});
