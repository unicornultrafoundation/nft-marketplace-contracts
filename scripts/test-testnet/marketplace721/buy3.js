...
const royaltiesByToken = [
  {
    account: addressAccount4,
    value: 700
  },
  {
    account: addressAccount5,
    value: 800
  }
];
...
const main = async () => {
  ...
  const contractRoyaltiesRegistrySigner1 = new ethers.Contract(ADDRESS_ROYALTIES_REGISTRY, abiRoyaltiesRegistry, signer1);
  const txSetRoyaltiesByToken = await contractRoyaltiesRegistrySigner1.setRoyaltiesByToken(ADDRESS_ERC721U2U, royaltiesByToken);
  await txSetRoyaltiesByToken.wait();

  const contractNFT721Signer6 = new ethers.Contract(ADDRESS_ERC721U2U, abiERC721U2U, signer6);
  const txApproveNFT721 = await contractNFT721Signer6.approve(ADDRESS_ERC721_NFT_MARKETPLACE, tokenId);
  await txApproveNFT721.wait();

  const contractERC721NFTMarketplaceSigner6 = new ethers.Contract(ADDRESS_ERC721_NFT_MARKETPLACE, abiERC721NFTMarketplace, signer6);
  const txCreateAsk721Marketplace = await contractERC721NFTMarketplaceSigner6.createAsk(ADDRESS_ERC721U2U, tokenId, quoteToken, price);
  await txCreateAsk721Marketplace.wait();

  const contractERC20Signer7 = new ethers.Contract(quoteToken, abiERC20, signer7);
  const txApproveERC20Signer7 = await contractERC20Signer7.approve(ADDRESS_ERC721_NFT_MARKETPLACE, totalPrice);
  await txApproveERC20Signer7.wait();

  const contractERC721NFTMarketplaceSigner7 = new ethers.Contract(ADDRESS_ERC721_NFT_MARKETPLACE, abiERC721NFTMarketplace, signer7);
  const txBuy721Marketplace = await contractERC721NFTMarketplaceSigner7.buy(ADDRESS_ERC721U2U, tokenId, quoteToken, totalPrice);
