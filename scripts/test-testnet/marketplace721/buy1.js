...
const buy2 = async () => {
  ...
  const contractNFT721Signer3 = new ethers.Contract(ADDRESS_ERC721U2U, abiERC721U2U, signer3);
  const txApproveNFT721 = await contractNFT721Signer3.approve(ADDRESS_ERC721_NFT_MARKETPLACE, tokenId);
  ...
  const txCreateAsk721Marketplace = await contractERC721NFTMarketplaceSigner3.createAsk(ADDRESS_ERC721U2U, tokenId, quoteToken, price);
  await txCreateAsk721Marketplace.wait();

  const contractERC20Signer5 = new ethers.Contract(quoteToken, abiERC20, signer5);
  const txApproveERC20Signer5 = await contractERC20Signer5.approve(ADDRESS_ERC721_NFT_MARKETPLACE, totalPrice);
  ...
  const txBuy721Marketplace = await contractERC721NFTMarketplaceSigner5.buy(ADDRESS_ERC721U2U, tokenId, quoteToken, totalPrice);
